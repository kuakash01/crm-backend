...
exports.handleGoogleOAuth = exports.verifyLoginOtp = exports.sendLoginOtp = exports.changePassword = exports.resetPassword = exports.forgotPassword = exports.acceptInvitation = exports.getInvitationDetails = exports.getCurrentUser = exports.login = exports.register = exports.verifyEmail = void 0;
...
function hashOtp(otp) {
    return crypto_1.default
        .createHmac("sha256", env_1.OTP_PEPPER)
        .update(otp)
        .digest("hex");
}
    ...
    try {
      ...
      `, [email]);
        ...
        if (userResult.rows.length === 0) {
            throw new AppError_1.AppError("Invalid verification request", 400);
        }
      ...
      `, [user.id]);
        if (challengeResult.rows.length === 0) {
            throw new AppError_1.AppError("Verification code is invalid or expired", 400);
        }
        ...
        if (challenge.attempts >=
            ...
            throw new AppError_1.AppError("Too many verification attempts. Please request a new code.", 429);
        }
    ...
    try {
      ...
      `, [userData.email]);
        ...
        `, [existing.id, otpHash]);
            ...
            await (0, emailTemplates_1.verifyEmailTemplate)(existing.email, otp);
            return {
                id: existing.id,
         ...
         * --------------------------------------------------
         */
        const hashedPassword = await bcryptjs_1.default.hash(userData.password, env_1.BCRYPT_SALT_ROUNDS
            ? parseInt(env_1.BCRYPT_SALT_ROUNDS)
            : 10);
      ...
      `, [user.id, otpHash]);
        ...
        await (0, emailTemplates_1.verifyEmailTemplate)(user.email, otp);
        return {
            id: user.id,
            organizationId: user.organization_id,
            fullName: user.fullname,
...
const login = async (email, password) => {
    try {
      ...
      `, [email]);
        ...
        throw new AppError_1.AppError("Failed to login user", 500);
    }
};
exports.login = login;
...
const getInvitationDetails = async (invitationToken) => {
    ...
    if (result.rows.length === 0) {
        throw new AppError_1.AppError("Invalid or expired invitation", 400);
    }
    const invitation = result.rows[0];
    if (new Date(invitation.expires_at) <=
        new Date()) {
        throw new AppError_1.AppError("This invitation has expired", 400);
    }
    return {
...
const acceptInvitation = async (invitationToken, password) => {
    ...
    try {
        await client.query("BEGIN");
        if (!invitationToken) {
            throw new AppError_1.AppError("Invitation token is required", 400);
        }
        if (!password) {
        ...
        if (invitationResult.rows.length === 0) {
            throw new AppError_1.AppError("Invalid invitation", 400);
        }
        const invitation = invitationResult.rows[0];
        // 3. Check if already used
        if (invitation.used_at) {
            throw new AppError_1.AppError("This invitation has already been used", 400);
        }
        // 4. Check expiration
        if (new Date(invitation.expires_at) <=
            new Date()) {
      ...
      `, [invitation.email]);
        if (existingUser.rows.length > 0) {
            throw new AppError_1.AppError("An account already exists for this email", 409);
        }
        // 6. Hash password
        const hashedPassword = await bcryptjs_1.default.hash(password, env_1.BCRYPT_SALT_ROUNDS
            ? parseInt(env_1.BCRYPT_SALT_ROUNDS)
            : 10);
      ...
      `, [invitation.id]);
        ...
        throw new AppError_1.AppError("Failed to accept invitation", 500);
    }
    finally {
        client.release();
    }
...
const forgotPassword = async (email) => {
    ...
    try {
      ...
      `, [user.id, otpHash]);
        await client.query("COMMIT");
        await (0, emailTemplates_1.sendPasswordResetEmail)(user.email, otp);
    }
    catch (error) {
        try {
            await client.query("ROLLBACK");
        ...
        throw new AppError_1.AppError("Failed to process password reset request", 500);
    }
    finally {
        client.release();
    }
...
const resetPassword = async (email, otp, newPassword) => {
    ...
    try {
      ...
      `, [email]);
        ...
        if (challenge.attempts >=
            challenge.max_attempts) {
            throw new AppError_1.AppError("Too many attempts. Please request a new code.", 429);
        }
        // 5. Verify OTP
        const submittedOtpHash = hashOtp(otp);
        if (submittedOtpHash !==
        ...
        // 6. Hash new password
        const hashedPassword = await bcryptjs_1.default.hash(newPassword, env_1.BCRYPT_SALT_ROUNDS
            ? parseInt(env_1.BCRYPT_SALT_ROUNDS)
            : 10);
        // 7. Update password
        await client.query(`
...
const sendLoginOtp = async (email) => {
    ...
    `, [normalizedEmail]);
    if (userResult.rows.length === 0) {
        throw new AppError_1.AppError("No account found with this email address. Please register first.", 404);
    }
    const user = userResult.rows[0];
    if (!user.is_active) {
        throw new AppError_1.AppError("Account is inactive. Please contact support.", 403);
    }
    // Rate limit: check if a code was sent in the last 60 seconds
    const recentChallenge = await db_1.pool.query(`
    ...
    if (recentChallenge.rows.length > 0) {
        throw new AppError_1.AppError("A code was recently sent. Please wait 60 seconds before requesting a new one.", 429);
    }
    ...
    try {
        await (0, emailTemplates_1.sendLoginOtpEmail)(normalizedEmail, otp);
    }
    catch (err) {
...
const verifyLoginOtp = async (email, otp) => {
    if (!email || !otp) {
        throw new AppError_1.AppError("Email and verification code are required", 400);
    }
    const normalizedEmail = email.trim().toLowerCase();
    ...
    try {
      ...
      `, [normalizedEmail]);
        if (userResult.rows.length === 0) {
            throw new AppError_1.AppError("Invalid login verification request", 400);
        }
        const user = userResult.rows[0];
        if (!user.is_active) {
            throw new AppError_1.AppError("Account is inactive", 403);
      ...
      `, [user.id]);
        if (challengeResult.rows.length === 0) {
            throw new AppError_1.AppError("Invalid or expired verification code. Please request a new code.", 400);
        }
        const challenge = challengeResult.rows[0];
        if (new Date(challenge.expires_at) <= new Date()) {
        ...
        if (challenge.attempts >= challenge.max_attempts) {
            throw new AppError_1.AppError("Too many invalid attempts. Please request a new code.", 429);
        }
      ...
      `, [challenge.id]);
        ...
        throw new AppError_1.AppError("Failed to verify login", 500);
    }
    finally {
        client.release();
    }
...
const handleGoogleOAuth = async (code) => {
    if (!env_1.GOOGLE_CLIENT_ID || !env_1.GOOGLE_CLIENT_SECRET) {
        throw new AppError_1.AppError("Google OAuth is not configured on the server. Please set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in backend/.env", 500);
    }
    ...
    if (!tokenResponse.ok || !tokenData.access_token) {
        ...
        throw new AppError_1.AppError(tokenData.error_description || "Failed to authenticate with Google", 400);
    }
    ...
    if (!userinfoResponse.ok || !profile.email) {
        ...
        throw new AppError_1.AppError("Failed to retrieve profile from Google", 400);
    }
    ...
    try {
      ...
      `, [email]);
        ...
        `, [roleId]);
            ...
            const randomPassword = "GAuth_" + crypto_1.default.randomBytes(16).toString("hex") + "9A!";
            const hashedPassword = await bcryptjs_1.default.hash(randomPassword, env_1.BCRYPT_SALT_ROUNDS ? parseInt(env_1.BCRYPT_SALT_ROUNDS) : 10);
            // Create User
        ...
        throw new AppError_1.AppError("Failed to complete Google authentication", 500);
    }
    finally {
        client.release();
    }
