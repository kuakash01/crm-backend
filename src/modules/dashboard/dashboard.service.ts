...
export const getDashboardStats = async (
  ...
  pipelineResult.rows.forEach((row) => {
    pipeline[row.stage as keyof typeof pipeline] = Number(row.total);
  });
  ...
  const conversionMetrics = [
    {
      ...
      current: roundMetric(
        rate(
          Number(conversion.converted_current),
          Number(conversion.leads_current)
        )
      ),
      previous: roundMetric(
        rate(
          Number(conversion.converted_previous),
          Number(conversion.leads_previous)
        )
      ),
      unit: "%",
      isPercentage: true,
    ...
    },
    {
      ...
      current: roundMetric(
        rate(
          Number(conversion.customers_active),
          Number(conversion.customers_total)
        )
      ),
      previous: roundMetric(
        rate(
          Number(conversion.customers_active_previous),
          Number(conversion.customers_total_previous)
        )
      ),
      unit: "%",
      isPercentage: true,
